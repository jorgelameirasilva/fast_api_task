    ...
    def insert_message(self, role: str, content: str, index: int = 1):
        ...
        if role == "user":
            message = ChatCompletionUserMessageParam(
                role="user", content=self.normalize_content(content)
            )
        elif role == "system":
            message = ChatCompletionSystemMessageParam(
                role="system", content=self.normalize_content(content)
            )
        elif role == "assistant":
            message = ChatCompletionAssistantMessageParam(
                role="assistant", content=self.normalize_content(content)
            )
        else:
