...
async def test_chat_endpoint_with_session(
    ...
    mock_session_service.get_session_messages = AsyncMock(
        return_value=[]
    ...
    mock_session_service.add_message = AsyncMock(return_value=mock_message)
...
async def test_chat_conversation_history_storage(
    ...
    mock_session_service.get_session_messages = AsyncMock(
        return_value=mock_existing_messages
    )

    # Mock add_message to return new messages
    mock_new_message = Mock()
    mock_new_message.id = "new-message-123"
    mock_session_service.add_message = AsyncMock(return_value=mock_new_message)
